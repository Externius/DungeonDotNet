...
public class Dungeon(IDungeonHelper dungeonHelper) : IDungeon
{
    ...
    private void AddToOpenList(DungeonTile node, int x, int y, ICollection<DungeonTile> openList,
    ...
    protected virtual bool CheckTileForOpenList(int x, int y)
    {
        return DungeonTiles[x][y].H != 0 && DungeonTiles[x][y].Texture != Texture.Room &&
               DungeonTiles[x][y].Texture != Texture.RoomEdge; // check it's not edge/room/room_edge
    }
    ...
    private int[] SetTilesForRoom()
    {
        ...
        return failSafeCount > 0 ? [x, y, right, down] : [0, 0, 0, 0]; // it can never be 0 if it's a good coordinate
    }
